...
class Auditor:
    ...
    def __init__(self, X_prime, y, fairness_def):
        ...
        if fairness_def == 'DP':
            # Our (authors of Fairness with overlapping groups) modification to make this compatible with
            # demographic parity
            cond_y = pd.Series(np.zeros(len(y)))
            fairness_def = 'FP'
        else:
            cond_y=y
        self.y_input = y
        ...
        if self.fairness_def not in ['DP', 'FP', 'FN']:
    ...
    def update_costs(self, c_0, c_1, group, C, iteration, gamma):
        ...
        for i in range(n):
            ...
            if self.y[i] == 0:
                ...
                if self.fairness_def == 'FP':
                    c_1[i] = (c_1[i]) * ((iteration-1.0)/iteration) + new_group_cost + (1.0 if self.y_input[i]==0 else -1.0)*1.0/(n*iteration)
                elif self.fairness_def == 'FN':
                ...
                if self.fairness_def == 'FP':
                    c_1[i] = (1.0 if self.y_input[i]==0 else -1.0)/(n)
                elif self.fairness_def == 'FN':
                    c_0[i] = -1.0/n
        return c_0, c_1
